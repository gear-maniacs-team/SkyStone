 ...
 */
class RoadrunnerOdometry(
    private val encoder: Encoder,
    wheelPoses: List<Pose2d>
) : Localizer, IUpdatable {
    ...
    override fun update() {
        ...
        // RobotPos
        RobotPos.currentX = poseEstimate.x
        RobotPos.currentY = poseEstimate.y
        RobotPos.currentAngle = poseEstimate.heading
    }

    /**
     * Returns the positions of the tracking wheels in the desired distance units (not encoder counts!)
     */
    fun getWheelPositions() = listOf(
        Encoder.ticksToCM(encoder.back.currentPosition.toDouble()),
        -Encoder.ticksToCM(encoder.left.currentPosition.toDouble()),
        Encoder.ticksToCM(encoder.right.currentPosition.toDouble())
    )
}
